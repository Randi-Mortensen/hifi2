...
module.exports = {
    'GET': function (req, res) {
        ...
        // Hvis params.catid, så hentes artikler med cat_id

        if (params.catid) {
            cond = 'category_id';
            values = [params.catid];
        }
        // Hvis params.artid, så hentes artiklen med artid
        if (params.artid) {
            cond = 'id';
            values = [params.artid];
        }
    ...
    'POST': function (req, res) {
        ...
        database.verifySession(res, cookie, function (veriFyData) {
            if (helpers.objEmpty(veriFyData)) {
                ...
                helpers.redirect(res, '/login'); // ...derfor sendes en rediect tilbage til browseren...
